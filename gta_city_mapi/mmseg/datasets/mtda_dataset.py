...
class MTDADataset(object):

    def __init__(self, source, target, target2, cfg):
        self.source = source
        self.target = target
    ...
    def synchronized_crop(self, s1, s2, s3):
    ...
    def get_rare_class_sample(self):
        ...
        i3 = np.random.choice(range(len(self.target2)))
        s3 = self.target2[i3]
        ...
        out = {
            **s1, 
            ...
            'target_img': s2['img'],
            'target2_img_metas': s3['img_metas'],
            'target2_img': s3['img']
        }
        if 'valid_pseudo_mask' in s2:
            out['valid_pseudo_mask'] = s2['valid_pseudo_mask']
        if 'valid_pseudo_mask' in s3:
            out['valid_pseudo_mask2'] = s3['valid_pseudo_mask']
        return out
    ...
    def __getitem__(self, idx):
        if self.rcs_enabled:
            ...
            s3 = self.target2[idx % len(self.target2)]
            s1, s2, s3 = self.synchronized_crop(s1, s2, s3)
            out = {
                **s1, 
                'target_img_metas': s2['img_metas'],
                'target_img': s2['img'],
