...
Our goal is to train one source domain and two target domains. So we have to modify the data loading process and how they are sent into the model for training. To achieve this, we have to modify the `forward_train` function in the decorator `mtda_seg/mmseg/models/uda/uda_decorator.py` and the model files under `mtda_seg/mmseg/models/uda/` (the `forward_train` functions).
